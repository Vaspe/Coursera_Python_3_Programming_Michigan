...
# trying out the EMBOSS filter
emboss_image = image.filter(PIL.ImageFilter.EMBOSS)
display(emboss_image)


# In[ ]:


# trying out the SHARPEN filter
...
#boxblur function
boxblur_image = image.filter(PIL.ImageFilter.BoxBlur(10))
display(boxblur_image)
...
medfilt_image = image.filter(PIL.ImageFilter.MedianFilter(size=3))
display(medfilt_image)
...
# 150 pixels
display(image.crop((45,0,190,142)))
...
drawing_object.rectangle((45,0,190,142), fill = None, outline ='red')
display(image)
...
# Now, lets iterate over our images. Except, we don't want to bother with the first one, because it is
